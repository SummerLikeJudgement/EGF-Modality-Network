...
class EMOE():
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        # 学习调度器，当验证集损失不再下降时降低学习率，耐心值为patience，降低因子为0.5
        ...
        # 如果评估指标是Loss希望Loss越小越好 → 'min'；如果评估指标是准确率等希望数值越大越好 → 'max'
        ...
        while True: # 早停控制循环
            ...
            with tqdm(dataloader['train']) as td: # 使用tqdm包装训练数据加载器，显示进度条
                for batch_data in td:
                    ...
                    # 损失计算
                    y_pred.append(output['logits_c'].cpu()) # append (batch_size, 5)
                    ...
                    for i,_ in enumerate(v_dist):
                        ...
                        dist[i][1] = (1/(gsr_dist[i]+0.1)) / s
                        dist[i][2] = (1/(v_dist[i]+0.1)) / s
                    ...
                    if self.args.fusion_method == "sum":
                        loss_ud = uni_distill(output['c_proj'], # 学生->多模态融合特征
                                              (output['ecg_proj'] * w[:,0].view(-1, 1) + output['gsr_proj'] * w[:,1].view(-1, 1) + output['v_proj'] * w[:,2].view(-1, 1)).detach() # 教师->单模态特征
                                              )
    ...
    def do_test(self, model, dataloader, mode="VAL", return_sample_results=False, f=0):
        ...
        eval_results["Loss"] = round(eval_loss, 4) # 添加损失到结果
